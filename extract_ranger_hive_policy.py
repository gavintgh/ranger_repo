...
for i in range(len(result)):
   ...
   if result[i]['resources']:
      if result[i]['resources']['database']:
	     for k in range(len(result[i]['resources']['database']['values'])):
		    policy['database'][0]['values'].append(result[i]['resources']['database']['values'][j])
